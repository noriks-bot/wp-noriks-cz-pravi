...
function gck_get_bundle_offers( $product_id = null ) : array {
    ...
    foreach ( $rows as $row ) {
        ...
        $saving_text   = "Ukupna ušteda " . number_format($saving_amount, 0, '.', '') . " Kč";
        ...
        if ( $qty === 1 ) {
            $offers[$offer_id] = [
                ...
                "qty"    => $qty,
                "title"  => $title,
                "per"    => number_format($sale, 0, '.', ''),
                "total"  => number_format($sale, 0, '.', ''),
                "regular" => number_format($regular, 0, '.', ''), // cena_1
                "saving" => $saving_text,
                "p1"     => $p1,
                "p2"     => $p2,
                "tip"    => $tip,
        ...
        $offers[$offer_id] = [
            ...
            "qty"    => $qty,
            "title"  => $title,
            "per"    => number_format($per_item_fixed, 0, '.', ''),
            "total"  => number_format($sale, 0, '.', ''),
            "regular" => number_format($regular, 0, '.', ''), // cena_1
            "saving" => $saving_text,
            "p1"     => $p1,
...
function gck_split_attrs_color_size( array $custom_attrs ) : array {
    ...
    foreach ( $custom_attrs as $key => $attr ) {
        ...
        $is_color = ( strpos($hay, 'strana') !== false || strpos($hay, 'boja') !== false || strpos($hay, 'barva') !== false || strpos($hay, 'farb') !== false || strpos($hay, 'culoare') !== false || strpos($hay, 'color') !== false || strpos($hay, 'colour') !== false || strpos($hay, 'colore') !== false || strpos($hay, 'kolor') !== false || strpos($hay, 'szín') !== false || strpos($hay, 'szin') !== false || strpos($hay, 'ρώμ') !== false || strpos($hay, 'цвят') !== false );
...
function gck_cz_majice_phrase( int $n, bool $free = false, string $type = 'majica' ) : string {
    if ( $type === 'steznica' ) {
        $m100 = $n % 100;
        if ( $n === 1 ) { $noun = 'ortézu'; }
        elseif ( in_array( $n % 10, array( 2, 3, 4 ), true ) && ! in_array( $m100, array( 12, 13, 14 ), true ) ) { $noun = 'ortézy'; }
        else { $noun = 'ortéz'; }
        return $free ? ( $noun . ' zdarma' ) : $noun;
    }
    $m100 = $n % 100;

    if ( $type === 'bokserica' ) {
        if ( $n === 1 || ( in_array( $n % 10, array( 2, 3, 4 ), true ) && ! in_array( $m100, array( 12, 13, 14 ), true ) ) ) {
            $noun = 'boxerky';
        } else {
            $noun = 'boxerek';
        }
    ...
    if ( $type === 'carapa' ) {
        // Compression SOCKS: ponožku / ponožky / ponožek.
        if ( $n === 1 ) {
            $noun = 'ponožku';
...
function gck_render_bundle_selector() {
    ...
    if (
        ...
        has_term( array( 'orto-kompresijske-carape', 'ponozky' ), 'product_cat', $product_id )
        || ( stripos( (string) $product->get_slug(), 'ponozk' ) !== false )
        || ( stripos( (string) $product->get_name(), 'ponožk' ) !== false )
    ) {
        ...
        // KneeFix je steznica za koljeno, ne majica.
        $gck_garment = 'steznica';
    }
    ...
    //  - orto-ortopas / orto-kidsnest : single "Velikost" attribute, no colour (size selector only).
    ...
    if ( has_term( array( 'orto-cloath' ), 'product_cat', $product_id ) ) {
        $gck_offer_subs = array( 'Set do koupelny', 'Balíček pro celý dům', 'Balíček pro rodinu a přátele' );
    } else {
        $gck_offer_subs = array();
    }
    $gck_offer_badges = has_term( array( 'orto-cloath', 'orto-cloud' ), 'product_cat', $product_id )
        ? array( 1 => 'NEJPRODÁVANĚJŠÍ', 2 => 'NEJLEPŠÍ CENA' )
        : array();
    ...
    // extended to 3 garment groups (4 trička + 1 boxerky + 1 pár ponožek). Gated
    // strictly to this category. This site's save/sync collapses product
    // attributes to a single Barva/Velikost, so the 3 groups' colour/size options
    ...
    if ( $gck_single_color ) {
        ...
        // Samo boja, prikazana kot krogci (brez selecta) — NORIKS Snug.
    ...
    // gratis-style sections instead of interleaved pairs. Saved data stays identical.
...
.color-hnědá {
  ...
  margin-right:6px;
    ...
    <?php
    // SHGIFTS: the ponožky size labels ("39-43" …) are wider than shirt/boxer sizes,
    ...
    <?php endif; ?>
        ...
        <?php
    // Your extra conditional style block (kept)
    if (  has_term( array( 'orto-starter', 'startovaci-balicek' ), 'product_cat', $product_id )  )   :
    ?>
        <style>
    ...
    <div class="gck-benefits-box">
        ...
        <ul class="gck-benefits-list">
            <?php if ( !has_term( array( 'orto-bokserice', 'orto-bokserice2', 'starter-paketi' ), 'product_cat', $product_id ) ) : ?>
                <li><span class="gck-check">✔</span> <strong>Perfektní padnutí</strong></li>
            <?php endif; ?>

            <li><strong>✔ Prémiové materiály a precizní střih</strong></li>
            <li><strong>✔ Pohodlí bez kompromisů</strong></li>

            <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
                <li style="color: #c00;"><strong>✔ Startovací balíček dostupný pouze jednou na osobu</strong></li>
                <li style="color: #c00;"><strong>✔ Limitováno na 1 000 balíčků </strong></li>
            <?php endif; ?>
        </ul>
        <?php endif; ?>
    ...
    <div class="gck-top-banner-wrap">
        <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
           ...
           <span>Pořiďte si svůj exkluzivní<br/> startovací balíček</span>
        </div>
    ...
    <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
        <div class="dev-banner" data-total="1000" data-sold="354">
            <div class="dev-banner__text">
                ...
                <span>Omezené množství <b class="sold">0</b> / <b class="total">0</b> (zbývá <b class="remain">0</b> ks)</span>
            </div>
    ...
    <?php if ( $show_countdown ) : ?>
        ...
        <div class="gck-countdown" id="gck-countdown"
            ...
            <div class="gck-countdown__body">
                Akce platí pouze dnes — ještě <span class="gck-countdown__timer" aria-live="polite">--:--</span>.<?php
                if ( $countdown_stock > 0 ) :
                    ?> Zbývá jen <strong><?php echo esc_html( $countdown_stock ); ?> ks</strong>.<?php
                endif; ?> <strong>Pospěšte si, než se vyprodá!</strong>
            </div>
        </div>
        <script>
          (function(){
    ...
    <?php if ( has_term( array( 'orto-snug', 'orto-kompwom', 'orto-pal' ), 'product_cat', $product_id ) ) :
    ?>
    <style>
    ...
    <?php if ( has_term( array( 'orto-snug' ), 'product_cat', $product_id ) ) :
    ?>
    <style>
      ...
      /* lokalizirane barve CZ */
      #bundle-selector .swatch-circle.color-modra { background: #b7c4e6 !important; }
      #bundle-selector .swatch-circle.color-ruzova { background: #e6a8b8 !important; }
      #bundle-selector .swatch-circle.color-seda { background: #c9c2ba !important; }
      #bundle-selector .swatch-circle.color-fialova { background: #b48fc4 !important; }
      #bundle-selector .swatch-circle.color-tmave-modra { background: #2b3a63 !important; }
    ...
    <?php if ( has_term( array( 'orto-kompwom' ), 'product_cat', $product_id ) ) :
    ?>
    <style>
      ...
      /* lokalizirane barve CZ */
      #bundle-selector .swatch-circle.color-cerna { background: #17161a !important; }
      #bundle-selector .swatch-circle.color-tmave-seda { background: #4a4a4f !important; }
      #bundle-selector .swatch-circle.color-ruzova { background: #e28ba4 !important; }
    ...
    <?php
    if ( has_term( array( 'orto-cloud', 'orto-snore' ), 'product_cat', $product_id ) ) :
    ?>
        <style>
          /* Kartica ponude 1:1 po referenci: slicica lijevo, naslov, podnaslov + postotak,
             pa cijena i precrtana stara cijena — sve lijevo poravnato. Radio je skriven. */
    ...
    (function(){
      var ORANGE = '#ff6d2e';
      var BORDER = <?php echo has_term( array( 'orto-kompwom' ), 'product_cat', $product_id ) ? "'#a8536b'" : "'#111111'"; ?>;   /* okvir izbornika: crn */
    ...
    <div id="bundle-selector" class="bundle-box<?php echo $gck_single_size ? ' is-single-size' : ''; ?><?php echo $gck_no_attrs ? ' is-no-attrs' : ''; ?>" data-split-garments="<?php echo $gck_split_garments ? '1' : '0'; ?>">
        ...
        foreach ( $offers as $offer_id => $data ) :
            ...
            // SHBOX split: real piece count is pairs * 2 (majice + bokserice).
            // SHGIFTS: real piece count is 6 (4 trička + 1 boxerky + 1 pár ponožek),
        ...
        ?>
            <label style="position: relative; <?php if ( ( ( $loop_index == 1 || $loop_index == 3 ) && ! $show_group_titles ) || ( ! empty( $gck_offer_badges ) && isset( $gck_offer_badges[ $loop_index ] ) ) ) : ?> margin-top: 25px;  <?php endif; ?>"
                ...
                <?php if ( ! $show_group_titles ) : ?>
                    <?php if ( $loop_index == 1 ) : ?>
                        <div class="gck-popular-badge">Nejoblíbenější 🔥</div>
                    <?php endif; ?>

                    <?php if ( $loop_index == 3 ) : ?>
                        <div class="gck-popular-badge-2">Nejnižší cena 🔥</div>
                    <?php endif; ?>
                <?php endif; ?>
    ...
    && !has_term( array( 'starter-paketi' ), 'product_cat', $product_id ) 
                ...
                <?php if ( $show_price_highlights ) : ?>
                    ...
                    <span class="gck-per-chip">
                        ...
                        <?php endif; ?>
                        <span class="gck-per-new"><?php echo number_format( $gck_per_new, 0 ); ?> Kč / ks</span>
                    </span>
                ...
                <div class="bundle-total-line">
                    ...
                    <span class="line-total"><?php echo number_format( (float) $data['total'], 0 ); ?>Kč</span>
                </div>

-->

<div class="bundle-total-line">
    <span style="font-weight:normal;">Celkem:</span>

    <?php if ( ! empty($data['regular']) && (float)$data['regular'] > (float)$data['total'] ) : ?>
        <span class="gck-regular-price">
            <?php echo number_format( (float) $data['regular'], 0 ); ?> Kč
        </span>
    <?php endif; ?>
    ...
    <span class="line-total"><?php echo number_format( (float) $data['total'], 0 ); ?> Kč</span>
                ...
                <div class="bundle-pairs <?php echo $is_default ? '' : 'hidden'; ?>"
                    ...
                    // attributes — "Barva trička"/"Velikost trička", "Barva boxerek"/
                    // "Velikost boxerek", "Barva ponožek"/"Velikost ponožek" — grouped by
                    ...
                    $gck_sections = array(
                        array(
                            'label'   => 'Vyber 4 ' . gck_cz_majice_phrase( 4, false, 'majica' ),
                            'gratis'  => false, 'count' => 4, 'gg' => 0, 'garment' => 'majica',
                            'ckey'    => 'majica_color', 'skey' => 'majica_size',
                            'colors'  => array( 'Černá', 'Bílá', 'Šedá', 'Béžová', 'Tmavě modrá', 'Hnědá', 'Zelená' ),
                            'sizes'   => array( 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL' ),
                        ),
                        array(
                            'label'   => 'Vyber 1 ' . gck_cz_majice_phrase( 1, false, 'bokserica' ) . ' zdarma',
                            'gratis'  => true, 'count' => 1, 'gg' => 1, 'garment' => 'bokserica',
                            'ckey'    => 'bokserica_color', 'skey' => 'bokserica_size',
                            'colors'  => array( 'Černá', 'Modrá', 'Šedá', 'Zelená', 'Červená' ),
                            'sizes'   => array( 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL' ),
                        ),
                        array(
                            'label'   => 'Vyber barvu ponožek (5 párů)',
                            'gratis'  => true, 'count' => 1, 'gg' => 2, 'garment' => 'carapa',
                            'ckey'    => 'carapa_color', 'skey' => 'carapa_size',
                            'colors'  => array( 'Černá', 'Bílá' ),
                    ...
                    foreach ( (array) $attr_groups as $grp ) {
                        ...
                        if ( strpos( $hay, 'majic' ) !== false || strpos( $hay, 'trick' ) !== false ) { $gck_real['majica']    = array( 'colors' => $cv, 'sizes' => $sv ); }
                        elseif ( strpos( $hay, 'bokseric' ) !== false || strpos( $hay, 'boxer' ) !== false ) { $gck_real['bokserica'] = array( 'colors' => $cv, 'sizes' => $sv ); }
                        elseif ( strpos( $hay, 'carap' ) !== false || strpos( $hay, 'ponozk' ) !== false || strpos( $hay, 'ponozek' ) !== false ) { $gck_real['carapa'] = array( 'colors' => $cv, 'sizes' => $sv ); }
                    }
                    ...
                    if ( $gck_split_garments ) {
                        $gck_passes = array(
                            array( 'group' => 0, 'label' => 'Vyber ' . (int) $pairs . ' ' . gck_cz_majice_phrase( $pairs, false, 'majica' ), 'gratis' => false ),
                            array( 'group' => 1, 'label' => 'Vyber ještě ' . (int) $pairs . ' ' . gck_cz_majice_phrase( $pairs, true, 'bokserica' ), 'gratis' => true ),
                        );
                    } else {
                    ...
                    <?php for ( $i = 1; $i <= $pairs; $i++ ) : ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_paid > 0 && $i === 1 ) : ?>
                            <div class="gck-pair-label">Vyber <?php echo (int) $gck_paid; ?> <?php echo esc_html( gck_cz_majice_phrase( $gck_paid, false, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_free > 0 && $i === ( $gck_paid + 1 ) ) : ?>
                            <div class="gck-pair-label is-gratis">Vyber ještě <?php echo (int) $gck_free; ?> <?php echo esc_html( gck_cz_majice_phrase( $gck_free, true, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <div class="bundle-pair">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateBundleUI(selectedRadio) {
        ...
        if (priceElem && total) {
            priceElem.innerHTML = total.replace('.', ',') + ' Kč';
        }
    }
 ...
 * total: tričko 1-4 (colour+size), boxerky (colour+size), ponožky (colour+size).
 * Each line is labelled by inspecting its field keys so the cart / order clearly
 * shows which garment was picked, e.g. "Tričko: Černá - L", "Boxerky: Modrá - M",
 * "Ponožky: Černá - 39-43".
 */
function gck_build_shgifts_lines( array $pairs_data ) : array {
    ...
    foreach ( $pairs_data as $attrs ) {
        ...
        foreach ( array_keys( $attrs ) as $k ) {
            ...
            if ( strpos( $kl, 'majic' ) !== false )    { $label = 'Tričko';  break; }
            if ( strpos( $kl, 'bokseric' ) !== false ) { $label = 'Boxerky'; break; }
            if ( strpos( $kl, 'carap' ) !== false )    { $label = 'Ponožky'; break; }
        }
...
function gck_add_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
    ...
    // SHGIFTS (orto-majica-darila): 6 items (4 trička + 1 boxerky + 1 pár ponožek),
...
function gck_shgifts_free_gifts_notice() {
    ...
    for ( $i = 0; $i < 5; $i++ ) {
        $gifts[] = array( 'label' => '1x pár ponožek', 'img' => $socks_img, 'price' => '329 Kč' );
    }
    ?>
    <style>
      .gck-fg{
    ...
    <div class="gck-fg">
        <div class="gck-fg__head">6 produktů ZDARMA ke každému nákupu</div>
        <div class="gck-fg__grid">
            <?php foreach ( $gifts as $g ) : ?>
                <div class="gck-fg__card">
                    <span class="gck-fg__badge">Zdarma</span>
