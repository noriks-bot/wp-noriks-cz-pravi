...
function gck_cz_majice_phrase( int $n, bool $free = false ) : string {
    $m100 = $n % 100;
    if ( $n === 1 ) {
        $noun = 'tričko';
    ...
    && !has_term( array( 'starter-paketi' ), 'product_cat', $product_id ) 
                ...
                <?php if ( $show_price_highlights ) : ?>
                    ...
                    <span class="gck-per-chip">
                        ...
                        <?php endif; ?>
                        <span class="gck-per-new"><?php echo number_format( (float) $data['per'], 0 ); ?> Kč / ks</span>
                    </span>
                    <?php if ( $discount_pct > 0 ) : ?>
                        <span class="gck-discount-badge">−<?php echo (int) $discount_pct; ?>%</span>
                    <?php endif; ?>
                <?php else : ?>
                    — <span class="bundle-option-title"><?php echo number_format( (float) $data['per'], 0 ); ?> Kč / ks</span>
                <?php endif; ?>
                ...
                <div class="bundle-pairs <?php echo $is_default ? '' : 'hidden'; ?>"
                    ...
                    <?php for ( $i = 1; $i <= $pairs; $i++ ) : ?>
                        <?php if ( $gck_show_sections && $gck_paid > 0 && $i === 1 ) : ?>
                            <div class="gck-pair-label">Vyber <?php echo (int) $gck_paid; ?> <?php echo esc_html( gck_cz_majice_phrase( $gck_paid ) ); ?></div>
                        <?php endif; ?>
                        <?php if ( $gck_show_sections && $gck_free > 0 && $i === ( $gck_paid + 1 ) ) : ?>
                            <div class="gck-pair-label is-gratis">Vyber ještě <?php echo (int) $gck_free; ?> <?php echo esc_html( gck_cz_majice_phrase( $gck_free, true ) ); ?></div>
                        <?php endif; ?>
