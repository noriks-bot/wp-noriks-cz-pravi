jQuery(function($){
  ...
  function canonicalKey(key){
    // remove WC prefixes and diacritics: e.g. "attribute_Velikost" -> "velikost"
  ...
  function updateCardMessages($wrap, $modal){
    ...
    $cards.each(function(){
      ...
      if(slots.length){
        $msg.text('Přidáno do slotu: ' + slots.join(', '));
      } else {
        $msg.empty();
      }
    });
    ...
    $wrap.find('.ibb-slot').each(function(i){
      ...
      if(filled){
        ...
        for(var k in attr){
          ...
          var ck = canonicalKey(k); // e.g. "attribute_Velikost" -> "velikost"
          if (ck === 'size' || ck === 'velikost' || ck === 'velkost' || ck === 'velicina') {
    ...
    $scope
      ...
      .each(function(){
        ...
        var key =
          ...
          $el.attr('name') || '';
        var ck = canonicalKey(key); // "velikost", "size", ...

        if (ck !== 'size' && ck !== 'velikost' && ck !== 'velkost' && ck !== 'velicina') return;
  ...
  $('.ibb-wrapper').each(function(){
    ...
    $modal.on('click', '.variation-buttons [data-value]', function(){
      ...
      var attrName = group.data('attribute-name'); // e.g. "velikost"
      ...
      if(type==='variable'){
        ...
        $.post(IBB.ajax,{action:'ibb_find_variation',nonce:IBB.nonce,product_id:pid,attributes:attrs},function(r){
          if(!r||!r.success||!r.data||!r.data.variation_id){
            $card.find('.ibb-msg').text(r&&r.data&&r.data.message?r.data.message:'Tato varianta není dostupná.');
            return;
          }
          finishSave({product_id:pid,variation_id:r.data.variation_id,attr:attrs,title:title,price_html:r.data.price||price,img:img});
        },'json').fail(function(){ $card.find('.ibb-msg').text('Chyba serveru.'); });
