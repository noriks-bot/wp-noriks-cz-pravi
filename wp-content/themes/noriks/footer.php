...
<!-- suport section -->
	...
	<?php
	// ===== Klaviyo newsletter =====
	// TODO(CZ): replace with the Czech footer list ID (Klaviyo -> Lists
	// & Segments -> Settings -> List ID). Left as placeholder so signups are
	// NOT sent to another market's list until the CZ list is set.
	$nf_klaviyo_list = 'WBtmtK';
	...
	if ( ! $nf_brand ) {
		$nf_brand = 'NORIKS je evropská značka výrobků pro každodenní použití. Zaměřujeme se na kvalitu, pohodlí a férové ceny — důvěřuje nám už více než 120 000 zákazníků.';
	}
	$nf_bg      = get_field( 'footer_bg_image', 'option' ); // background photo behind the brand band
	$nf_legal   = get_field( 'footer_legal_text', 'option' );
	if ( ! $nf_legal ) {
		$nf_legal = 'NORIKS BRAND. Všechna práva vyhrazena. Ceny jsou včetně DPH. Obrázky jsou ilustrativní.';
	}
	?>
		...
		<section class="nf-signup">
			<div class="nf-wrap nf-signup-grid">
				<div class="nf-signup-copy">
					<h2 class="nf-signup-title">Připoj se k rodině NORIKS</h2>
					<p class="nf-signup-sub">Exkluzivní nabídky a novinky o produktech — dozvíš se to první.</p>
				</div>
				<div class="nf-signup-form-col">
					<form class="nf-signup-form" data-klaviyo-list="<?php echo esc_attr( $nf_klaviyo_list ); ?>" novalidate>
						<label class="nf-field-label" for="nf-email">Tvůj e-mail *</label>
						<div class="nf-field-row">
							<input type="email" id="nf-email" name="email" class="nf-input" placeholder="Tvoje e-mailová adresa" required autocomplete="email">
							<button type="submit" class="nf-btn nf-btn-light">Přihlásit se</button>
						</div>
						<p class="nf-signup-note">
							Přihlášením souhlasíš se zasíláním e-mailů. Odhlásit se můžeš kdykoli.
							<a href="/cz/ochrana-osobnich-udaju/">Zásady ochrany osobních údajů.</a>
						</p>
				...
				<nav class="nf-links">
					<?php if ( $nf_col2_h || $nf_col2_l ) : ?>
						<div class="nf-link-col">
							...
							<?php if ( ! $nf_has_upute ) : ?>
								<a href="/cz/navody/">Návod k použití</a>
							<?php endif; ?>
						</div>
					<?php endif; ?>
		...
		<div class="nf-wrap nf-meta">
			<div class="nf-meta-left">
				<span class="nf-region">
					<img loading="lazy" decoding="async" src="https://static.devit.software/countries/flags/rectangle/cz.svg" alt="" class="nf-flag">
					Česko (CZ)
				</span>
				<span class="nf-copy">© <?php echo date( 'Y' ); ?> NORIKS. Všechna práva vyhrazena.</span>
			</div>
	...
	(function () {
		...
		form.addEventListener('submit', function (e) {
			...
			if (!email || email.indexOf('@') === -1) {
				msg.textContent = 'Zadej platnou e-mailovou adresu.'; msg.className = 'nf-signup-msg is-error'; return;
			}
			if (!list || list === 'REPLACE_LIST_ID') {
				msg.textContent = 'Newsletter zatím není propojen (chybí Klaviyo List ID).'; msg.className = 'nf-signup-msg is-error'; return;
			}
			msg.textContent = 'Přihlašování…'; msg.className = 'nf-signup-msg';
			...
			fetch('https://manage.kmail-lists.com/ajax/subscriptions/subscribe', {
				...
				if (d && d.success) {
					msg.textContent = 'Děkujeme! Zkontroluj e-mail pro potvrzení.'; msg.className = 'nf-signup-msg is-ok';
					form.reset();
				} else {
					msg.textContent = (d && d.errors && d.errors[0]) ? d.errors[0] : 'Něco se pokazilo, zkus to prosím znovu.';
					msg.className = 'nf-signup-msg is-error';
				}
			}).catch(function () {
				msg.textContent = 'Něco se pokazilo, zkus to prosím znovu.'; msg.className = 'nf-signup-msg is-error';
			});
		});
	})();
	</script>
