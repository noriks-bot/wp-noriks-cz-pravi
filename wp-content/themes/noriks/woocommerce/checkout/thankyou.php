 ...
 * Step 2: 6-product grid (after "Nechci" or after adding 1 item)
 *
...
foreach ( $order->get_items() as $item ) {
    ...
    $is_majica = ( strpos($cat_str, 'orto-majice') !== false || strpos($cat_str, 'tricka') !== false || strpos($name, 'trička') !== false || strpos($name, 'tričk') !== false || strpos($cat_str, 'majic') !== false || strpos($name, 'majic') !== false );
...
if ( $upsell_is_majice ) {
    ...
    $upsell_title_text = 'Přidejte trička teď – 50% sleva';
} else {
    $upsell_product_id = 4766; // Šedé boxerky (siva verzija, ista struktura kot crna)
    $upsell_name       = 'Šedé boxerky';
    $upsell_qty_prices = array( 1 => 119, 3 => 357, 5 => 595 );
    $upsell_qty_names  = array( 1 => '1x Šedé boxerky', 3 => '3x Šedé boxerky', 5 => '5x Šedých boxerek' );
    $upsell_qty_images = array(
        1 => 'https://noriks.com/cz/wp-content/uploads/2026/01/sive-boksarice-produktna.jpg',
        3 => 'https://noriks.com/cz/wp-content/uploads/2026/01/sive-boksarice-produktna.jpg', // TODO: 3x siva kompozitna, ko obstaja
        5 => 'https://noriks.com/cz/wp-content/uploads/2026/01/sive-boksarice-produktna.jpg', // TODO: 5x siva kompozitna, ko obstaja
    );
    $upsell_title_text = 'Přidejte boxerky teď – 50% sleva';
}
...
<div id="order-splash" style="position:fixed;top:0;left:0;width:100%;height:100%;background:#04ac00;z-index:999999;display:flex;flex-direction:column;align-items:center;justify-content:center;transition:opacity 0.6s ease;">
    ...
    <h1 style="color:#fff;font-family:'Roboto',sans-serif;font-size:28px;font-weight:700;margin:0;">Objednávka přijata!</h1>
...
<div class="ty-container">
    ...
    <?php if ( $order->has_status( 'failed' ) ) : ?>
        <div class="ty-success" style="background:#fde8e8;">
            ...
            <h1>Objednávka se nezdařila</h1>
            <p>Banka odmítla transakci. Zkuste to znovu.</p>
            <p style="margin-top:16px;">
                <a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>" style="display:inline-block;background:#E8450E;color:#fff;padding:12px 32px;text-decoration:none;font-weight:700;">Zkuste znovu</a>
            </p>
        </div>
    <?php else : ?>
        ...
        <div class="ty-success">
            ...
            <p>Potvrzení bylo odesláno na <?php echo esc_html( $order->get_billing_email() ); ?></p>
            <span class="ty-order-num">Objednávka #<?php echo $order->get_order_number(); ?></span>
        </div>

        <!-- ═══ STEP 1: VIGOSHOP UPSELL (COD only) ═══ -->
        <?php if ( $order->get_payment_method() === 'cod' && (float)$order->get_total() <= 3000 ) : ?>
            ...
            <div class="ty_upsell_one_wrapper__popup-content">
                ...
                <div class="tyuo_timer" style="position:relative;">
                    <div class="timer_wrapper">
                        <div class="special_offer_txt">Poslední šance – nabídka vyprší za</div>
                ...
                <div class="tyuo_middle_section">
                    <div class="sub_title">
                        ...
                        Bez dalšího poštovného – vše v jednom balíku
                    </div>
                    <div class="clue_text">
                ...
                <div class="tyuo_product_section">
                    ...
                    <div class="product_data">
                        ...
                        <div class="right_section_wrapper">
                            ...
                            <div class="product_regular_price" id="ty-upsell-regular"><?php echo number_format($upsell_qty_regular[1], 0, ',', ' '); ?> Kč</div>
                            <div class="product_new_sale_price" id="ty-upsell-price"><?php echo number_format($upsell_qty_prices[1], 0, ',', ' '); ?> Kč</div>
                        </div>
                    </div>

                    <script>
                    (function(){
                        var prices = <?php echo json_encode(array_map(function($p){ return number_format($p,0,',',' ') . ' Kč'; }, $upsell_qty_prices)); ?>;
                        var names = <?php echo json_encode($upsell_qty_names); ?>;
                        var images = <?php echo json_encode($upsell_qty_images); ?>;
                        var regulars = <?php echo json_encode(array_map(function($p){ return number_format($p,0,',',' ') . ' Kč'; }, $upsell_qty_regular)); ?>;
                        ...
                        <select class="variation-select" id="ty-variation-select">
                            <?php if ( $upsell_variations ) : ?>
                                ...
                                <option value="<?php echo $v['id']; ?>" <?php selected( strtolower($v['size']), strtolower($customer_size) ); ?>>
                                    Černá, <?php echo esc_html( $v['size'] ); ?>
                                </option>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <option value="">Černá, S</option>
                                <option value="">Černá, M</option>
                                <option value="">Černá, L</option>
                                <option value="">Černá, XL</option>
                                <option value="">Černá, XXL</option>
                            <?php endif; ?>
                        </select>
                    </div>
                    ...
                    <div class="buttons-section">
                        <a class="pass-btn" id="ty-btn-skip">Nechci</a>
                        <div class="buy-btn" id="ty-btn-add" data-product-id="<?php echo esc_attr( $upsell_product_id ); ?>">PŘIDAT K OBJEDNÁVCE</div>
                    </div>
                </div>
            ...
            <div class="ty-grid-popup">
                <div class="ty-grid-header" style="cursor:pointer;flex-direction:column;position:relative;">
                    ...
                    <div style="display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:8px;">
                        <div style="font-size:15px;font-weight:400;color:#000;">Speciální nabídka vyprší</div>
                    ...
                    <div style="font-size:20px;font-weight:700;color:#000;line-height:1.3;text-align:center;padding-top:10px;">Přidejte jakýkoli produkt s 50% slevou</div>
                </div>
                <div class="ty-section-body open" id="ty-grid-body">
                <div class="ty-section-body-inner" style="padding:0;">
                <div class="ty-grid-trust">
                    ✔ Vše posíláme v jednom balíku
                </div>
                <div class="ty-grid">
                    ...
                    <div class="ty-grid-item">
                        ...
                        <div class="g-price-old"><?php echo number_format( $gp_price, 0, ',', ' ' ); ?> Kč</div>
                        <div class="g-price-new"><?php echo number_format( $gp_sale, 0, ',', ' ' ); ?> Kč</div>
                        <?php if ( $gp_vars ) : ?>
                ...
                <div class="buttons-section">
                    <a class="pass-btn" id="ty-grid-close">Nechci</a>
                    <div class="buy-btn" id="ty-grid-add-all">PŘIDAT K OBJEDNÁVCE</div>
                </div>
        ...
        <div class="ty-section">
            ...
            <div class="ty-section-body open">
                <div class="ty-section-body-inner">
                    <div class="ty-row"><span class="ty-row-label">Jméno</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_first_name() . ' ' . $order->get_billing_last_name() ); ?></span></div>
                    ...
                    <div class="ty-row"><span class="ty-row-label">Město</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_postcode() . ' ' . $order->get_billing_city() ); ?></span></div>
...
(function(){
    ...
    function tick() {
        if (rem <= 0) {
            ...
            // Auto-open "Položky objednávky" and "Doručovací adresa" sections
        ...
        addBtn.addEventListener('click', function() {
            ...
            fetch(ajaxUrl, { method: 'POST', body: fd })
                ...
                .catch(function() {
                    ...
                    addBtn.textContent = 'PŘIDAT K OBJEDNÁVCE';
                });
        });
    }
    ...
    if (overlay) {
        ...
        overlay.querySelectorAll('.g-select-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                if (el.classList.contains('selected')) {
                    ...
                    el.textContent = 'VYBRAT';
                    el.style.background = '#000';
                } else {
                    el.classList.add('selected');
                    el.innerHTML = '✔ VYBRÁNO <span style="font-size:10px;opacity:0.7;margin-left:4px;">odstranit</span>';
                    el.style.background = '#2E7D32';
                }
            });
        });

        // "Nechci" — close all, show summary without upsells
        document.getElementById('ty-grid-close').addEventListener('click', closeAll);

        // "PŘIDAT K OBJEDNÁVCE" — add all selected items, then close
        ...
        if (gridAddAll) {
            gridAddAll.addEventListener('click', function() {
                ...
                gridAddAll.textContent = 'Přidávám...';
