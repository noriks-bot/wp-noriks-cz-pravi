 ...
 <div class="accordion">
    ...
    <div class="accordion-item">
      ...
      <div class="accordion-content">
         ...
         <?php elseif( function_exists('noriks_is_type') && noriks_is_type('kneeheat', $current_product_id) ): ?>

                <strong>NORIKS KneeHeat</strong> je bandáž na koleno, která v jednom sezení spojuje tři terapie: teplo do 42 °C, vzduchovou kompresi a vibrační masáž 60 Hz. Teplo rozšiřuje cévy a usnadňuje přísun krve do hlubší tkáně, rytmická komprese vytlačuje nahromaděnou tekutinu a vibrace uvolňují ztuhlost kolem kloubu.<br><br>Sezení trvá 12 minut a spouští se jedním tlačítkem — bez aplikace, bez nastavování a bez kabelu. K dispozici jsou 3 stupně tepla a 3 režimy vibrací, takže intenzitu přizpůsobíte dni a potížím.<br><br>Bandáž má ergonomický tvar a drží nastavitelnými pásky; sedí na levé i pravé koleno a v balení je i prodlužovací pásek pro větší obvody. Látka na kůži je měkká a prodyšná.<br><br>Přístroj se nabíjí přiloženým USB-C kabelem a jedno nabití vydrží na více sezení. K přístroji dostanete 2 roky záruku na výměnu.
        ...
        <?php echo get_field("singlepp_acc_t_1","options"); ?>
         ...
         <?php elseif( function_exists('noriks_is_type') && noriks_is_type('controlpro', $current_product_id) ): ?>
                ...
                NORIKS ControlPro je mechanický trenažér pánevního dna se skutečným fyzickým odporem. Místo stlačování naprázdno dostanete něco, proti čemu můžete tlačit — stejný princip postupné zátěže, kterým sílí každý jiný sval v těle.<br><br>Používá se vsedě: přístroj vložíte mezi kolena a stlačujete proti odporu — 3 série po 10 opakováních denně, přibližně pět minut. Bez sond a zavádění, bez kabelů, gelu, baterií a aplikací. Vestavěné počítadlo opakování ukazuje, kolik jste odcvičili.<br><br>Konstrukce je pevná, s měkkými podložkami na opěrkách, které netlačí ani po několika sériích. Odpor se postupně zvyšuje, jak svaly sílí.<br><br>Je určen mužům, kteří chtějí získat zpět kontrolu nad měchýřem — po operaci prostaty, při úniku během námahy nebo po dlouhých měsících Kegelových cviků bez výsledku. Doručení je diskrétní, v neutrální krabici bez označení.
...
<br>
     ...
     <?php if ( ! ( function_exists('noriks_is_type') && ( noriks_is_type('bunion', $current_product_id) || noriks_is_type('fisiorest', $current_product_id) || noriks_is_type('norikshers', $current_product_id) || noriks_is_type('ortopedski-jastuk', $current_product_id) ) )  && ! ( function_exists('noriks_is_type') && noriks_is_type('kneefix', $current_product_id) || noriks_is_type('controlpro', $current_product_id) ) && ! ( function_exists('noriks_is_type') && noriks_is_type('snug', $current_product_id) ) && ! ( function_exists('noriks_is_type') && noriks_is_type('pal', $current_product_id) ) && ! ( function_exists('noriks_is_type') && noriks_is_type('red', $current_product_id) || noriks_is_type('kneeheat', $current_product_id) )) : // žádná tabulka velikostí pro bunion + fisiorest + norikshers + ortopedicky polstar ?>
     <div class="accordion-item">
      <div class="accordion-header" onclick="toggleAccordion(this)">
    ...
    <!-- 3 - savjeti za pranje-->
    <?php if ( ! ( function_exists('noriks_is_type') && ( noriks_is_type('ortopas', $current_product_id) || noriks_is_type('bunion', $current_product_id) || noriks_is_type('fisiorest', $current_product_id) || noriks_is_type('norikshers', $current_product_id) || noriks_is_type('kidsnest', $current_product_id) || noriks_is_type('ortopedski-jastuk', $current_product_id) ) )  && ! ( function_exists('noriks_is_type') && noriks_is_type('kneefix', $current_product_id) || noriks_is_type('controlpro', $current_product_id) )) : // žádné tipy na praní pro pás/bunion/fisiorest + norikshers + kidsnest + ortopedicky polstar ?>
    <div class="accordion-item">
      <div class="accordion-header" onclick="toggleAccordion(this)">
