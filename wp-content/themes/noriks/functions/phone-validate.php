 ...
 *
 * Poleg tega stevilko ob oddaji naročila tiho pretvori v mednarodno obliko (+420...),
 * ker je Metakocka del narocil zavracala samo zaradi presledkov in oklepajev.
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'NORIKS_TEL_CC',    '420' );
...
add_action( 'woocommerce_checkout_process', function () {
    ...
    if ( $p !== '' && ! noriks_tel_ok( $p ) ) {
        wc_add_notice( 'Zkontrolujte telefonní číslo — zdá se, že není úplné.', 'error' );
    }
}, 5 );
...
add_action( 'wp_footer', function () {
    ...
    jQuery(function($){
      var CC = '420', TRUNK = '', MIN = 9, MAX = 12;
      var MSG = <?php echo wp_json_encode( 'Zkontrolujte telefonní číslo — zdá se, že není úplné.' . ' ' . 'např. 601 234 567' ); ?>;
