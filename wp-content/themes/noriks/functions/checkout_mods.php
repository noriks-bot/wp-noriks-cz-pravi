    ...
    jQuery(function($){
      ...
      function validateField(field, force) {
        ...
        if (isPhone && val && val.replace(/\D/g,'').length < 6) {
          showError($row, '\u2715 Zadejte platné telefonní číslo');
          return false;
        }
...
add_action( 'woocommerce_cart_calculate_fees', function( $cart ) {
    ...
    if ( $chosen_gateway === 'cod' ) {
        $cart->add_fee( 'Dobírka', 1.99, false );
    }
});
...
add_action('woocommerce_review_order_before_submit', function(){
    ...
    <div class="noriks-coupon-wrap" style="margin:12px 0 16px;">
        ...
        <div id="noriks-coupon-expanded" style="display:none;gap:8px;align-items:center;">
            ...
            <button type="button" style="padding:10px 20px;background:#000;color:#fff;border:none;border-radius:6px;font-size:14px;font-weight:600;cursor:pointer;white-space:nowrap;" onclick="noriksApplyCoupon()">Použít</button>
    ...
    function noriksApplyCoupon(){
        ...
        fetch('<?php echo esc_url(wc_get_checkout_url()); ?>?wc-ajax=apply_coupon',{
            ...
            var isError=!res.ok||res.html.indexOf('error')!==-1||res.html.indexOf('neexistuje')!==-1||res.html.indexOf('není platný')!==-1||res.html.indexOf('removed')!==-1;
            if(isError){
                msg.style.background='#fde8e8';msg.style.color='#c00';
                var txt=res.html.replace(/<[^>]*>/g,'').trim();
                msg.innerHTML='❌ '+(txt||'Kód kupónu není platný.');
            }else{
                msg.style.background='#e8fde8';msg.style.color='#080';
                msg.innerHTML='✅ Kupón použit!';
                document.getElementById('noriks_coupon_code').value='';
                if(window.jQuery)jQuery('body').trigger('update_checkout');
            }
            btn.textContent='Použít';btn.disabled=false;
        }).catch(function(){
            msg.style.display='block';msg.style.background='#fde8e8';msg.style.color='#c00';
            msg.textContent='Chyba. Zkuste to znovu.';btn.textContent='Použít';btn.disabled=false;
        });
    }
    </script>
    <?php
