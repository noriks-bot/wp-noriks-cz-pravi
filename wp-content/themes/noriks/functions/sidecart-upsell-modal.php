...
function noriks_ajax_add_to_cart() {
    ...
    if ($cart_item_key) {
        ...
        $msg = !empty($notices) ? strip_tags($notices[0]['notice'] ?? $notices[0]) : 'Chyba pri dodavanju';
        wc_clear_notices();
...
function noriks_upsell_modal_markup() {
        ...
        <div class="noriks-modal">
            ...
            <div class="noriks-modal-body">
                ...
                <div class="noriks-modal-qty-row">
                    <span class="noriks-attr-label">MNOŽSTVÍ</span>
                ...
                <div id="noriks-modal-error" class="noriks-modal-error" style="display:none;">Vyberte všechny možnosti</div>
                <button id="noriks-modal-add" class="noriks-modal-add-btn">PŘIDAT DO KOŠÍKU</button>
            </div>

        </div>
    </div>
    <style>
        /* Side cart button overrides — force order + colors (override inline styles) */
        .xoo-wsc-footer { padding-top:0 !important; padding-bottom:30px !important; }
        .xoo-wsc-ft-buttons-cont { display:flex !important; flex-direction:column !important; gap:8px !important; margin-top:0 !important; padding-top:0 !important; }
        .xoo-wsc-ft-buttons-cont a[class*="checkout"] { order:-1 !important; background:#c00 !important; background-color:#c00 !important; color:#fff !important; border:none !important; border-radius:4px !important; padding:22px 20px !important; margin:0 !important; font-size:20px !important; font-weight:700 !important; text-align:center !important; width:100% !important; box-sizing:border-box !important; }
        .xoo-wsc-ft-buttons-cont a[class*="continue"] { order:1 !important; background:#fff !important; background-color:#fff !important; color:#000 !important; border:1px solid #000 !important; border-radius:4px !important; padding:8px 20px !important; font-size:75% !important; text-align:center !important; width:100% !important; box-sizing:border-box !important; }
        .xoo-wsc-ft-buttons-cont a[class*="continue"]:hover { background:#f5f5f5 !important; }
        /* Override ANY inline styles on buttons */
        .xoo-wsc-ft-buttons-cont a { display:block !important; }
    ...
    (function($) {
        ...
        function renderModal() {
            ...
            $('#noriks-modal-add').text('PŘIDAT DO KOŠÍKU').removeClass('adding added');
        ...
        $(document).on('click', '#noriks-modal-add', function() {
            ...
            if (!variation) {
                $('#noriks-modal-error').text('Tato kombinace není dostupná').show();
                return;
            }

            if (!variation.is_in_stock) {
                $('#noriks-modal-error').text('Není skladem').show();
                return;
            }

            $btn.addClass('adding').text('PŘIDÁVÁM...');
            ...
            $.post(woocommerce_params.ajax_url, data, function(res) {
                ...
                if (res.success !== false && res.fragments) {
                    $btn.removeClass('adding').addClass('added').text('✓ PŘIDÁNO!');
                    ...
                    $btn.removeClass('adding').text('PŘIDAT DO KOŠÍKU');
                    var msg = (res.data && res.data.message) ? res.data.message : 'Chyba pri dodavanju';
                ...
                $btn.removeClass('adding').text('PŘIDAT DO KOŠÍKU');
                $('#noriks-modal-error').text('Chyba pri dodavanju').show();
            });
        });
